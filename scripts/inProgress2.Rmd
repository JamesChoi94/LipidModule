---
title: "Lipid gene module detection"
author: "James Choi"
...
results_out <- '../results/LipidGeneModule/'
...
## WGCNA on Lee lab macrophage RNAseq

```{r}
counts <- list(invitro, ribotag)
...
```{r}
remove_genes <- rowSums(counts < 10) > 12
# gene summaries
...
table(samples_pass)
counts <- counts[gene_pass & !remove_genes, samples_pass]
...
tmp <- prcomp(x = counts)
pc <- c(1,2)
plot(tmp$x[,pc[1]], tmp$x[,pc[2]],
     pch = 16, cex = 2, xlab = paste('PC', pc[1]), ylab = paste('PC', pc[2]))
text(x = tmp$x[,pc[1]], y = tmp$x[,pc[2]], 
     label = colnames(counts), cex = 1)
tmp$x %>% 
  as.data.frame() %>% 
...
### Load Chen et al SCI RNAseq

```{r}
dlcheck <- download.file(
  url = 'https://www.ncbi.nlm.nih.gov/geo/download/?acc=GSE45376&format=file',
  destfile = '../data/GSE45376_RAW.tar',
  mode = 'wb' # the TAR file is a binary
)
if (dlcheck != 0) {
  stop('GSE45376 download unsuccessful')
} else {
  untar(tarfile = '../data/GSE45376_RAW.tar',
        exdir = '../data/GSE45376_RAW/')
}
chen_counts <- list()
chen_files <- list.files(path = '../data/GSE45376_RAW/', 
                         full.names = TRUE,
                         pattern = 'gz$')
for (i in 1:length(chen_files)) {
  R.utils::gunzip(chen_files[i])
}
chen_files <- list.files(path = '../data/GSE45376_RAW/',
                         full.names = TRUE,
                         pattern = 'xls$')
for (i in 1:length(chen_files)) {
  chen_counts[[i]] <- readxl::read_xls(path = chen_files[i])
}
```
