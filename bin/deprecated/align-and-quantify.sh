...
# Validate HISAT2 index files and directories
if [[ ! $(find ${HISAT2_INDEX_DIR} -name "${HISAT2_INDEX_BASENAME}*") ]]
then
	echo "Cannot find provided HISAT2 index directory\n"
	echo "or HISAT2 index files with basename ${HISAT2_INDEX_BASENAME}"
	
else
mkdir ${LIPID_HOME}/ref/HISAT2_index
# Download UCSC mm10 genome index
